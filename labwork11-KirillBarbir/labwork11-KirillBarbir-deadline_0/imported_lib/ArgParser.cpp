        ...
        if (input_arguments[i].size() <= 1 || input_arguments[i][0] != '-') {
            ...
            switch (arg_type) {
                case (ArgType::kString):
                    if (!string_opts_[ind].option_info.is_positional || string_opts_[ind].option_info.positional_order != current_positional) {
                        return false;
                    }
                ...
                case (ArgType::kInteger):
                    ...
                    if (!int_opts_[ind].option_info.is_positional || int_opts_[ind].option_info.positional_order != current_positional) {
                        return false;
                    }
...
std::pair<size_t, ArgType> ArgParser::FindPositionalOption() {
    for (int i = 0; i < string_opts_.size(); ++i) {
        if (!string_opts_[i].option_info.is_positional || string_opts_[i].option_info.positional_order != current_positional) {
            continue;
        }
        return {i, ArgType::kString};
    }
    ...
    for (int i = 0; i < int_opts_.size(); ++i) {
        if (!int_opts_[i].option_info.is_positional || int_opts_[i].option_info.positional_order != current_positional) {
            continue;
        }
        return {i, ArgType::kInteger};
    }
